...
public class TestURLUtil {
	...
	@Test
	public void testGenFileNameFromURL() {
		String timeStamp = TimeUtil.getTimeStamp();
		assertEquals(String.format("speedtest_tele2_net-d-%s.txt", timeStamp), URLUtil.genFileNameFromURL(links.get(0)));
		assertEquals(String.format("localhost-h-%s.txt", timeStamp), URLUtil.genFileNameFromURL(links.get(1)));
		assertEquals(String.format("127_0_0_1-dddd-%s.txt", timeStamp), URLUtil.genFileNameFromURL(links.get(2)));
		assertEquals(null, URLUtil.genFileNameFromURL(links.get(3)));
		assertEquals(null, URLUtil.genFileNameFromURL(links.get(4)));
		assertEquals(null, URLUtil.genFileNameFromURL(links.get(5)));
		assertEquals(null, URLUtil.genFileNameFromURL(links.get(6)));
	}
	...
	@Test
	public void testNullInputOnGenFileNameFromURL() {
		expEx.expect(NullPointerException.class);
		URLUtil.genFileNameFromURL(links.get(7));
	}
